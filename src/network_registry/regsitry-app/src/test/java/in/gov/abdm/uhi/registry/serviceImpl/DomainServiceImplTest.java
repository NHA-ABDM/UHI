...
class DomainServiceImplTest {
	...
	public void loadData() throws JsonMappingException, JsonProcessingException {
		readValue = mapper.readValue(payload, new TypeReference<List<Domains>>() {
		});

	}

	@Test
	public void shouldGetAllDomain() throws Exception {
		when(domainRepository.findAll()).thenReturn(readValue);
		assertEquals(readValue.size(), domainServiceImpl.findAllDomain().size());
	}

	@Test
	public void testSaveDomain() {
		// given - precondition or setup //
		when(domainRepository.saveAll(readValue)).thenReturn(readValue);
		assertEquals(readValue, domainServiceImpl.saveAllDomain(readValue));
	}
