...
public class NetworkRegistryService {
	...
	Mono<String> validateParticipant(Request request, Map<String, String> headers, String req, String subs2) {
		Mono<String> result = null;
		try {
			Subscriber subs = appConfig.objectMapper().readValue(subs2, Subscriber.class);
			if (!request.getContext().getConsumerId().equalsIgnoreCase(subs.getSubscriber_id())) {
				...
				try {
					if (signatureUtility.verifySign(request, headers, req, subs)) {
	...
	public Mono<String> circuitBreakerWrapper(Request body) throws JsonProcessingException {
	...
	private Mono<String> sendSingletoLookup(Request body) throws JsonProcessingException {
