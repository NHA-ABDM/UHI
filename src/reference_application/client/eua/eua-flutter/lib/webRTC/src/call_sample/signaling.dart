...
class Signaling {
  String _roomId = 'sana.bhatt';
  Signaling(this._host, {this.doctorsHPRAdd}) {
    _roomId = doctorsHPRAdd;
  }
  ...
  getPatientId() async {
    await SharedPreferencesHelper.getABhaAddress()
        .then((value) => _selfId = value!);
  }
      ...
      case 'roomCreated':
        {
          await getPatientId();
          _roomId = data['roomId'];
          log("$_roomId", name: "ROOM ID");
          final prefs = await SharedPreferences.getInstance();
          String? name = prefs.getString(AppStrings().chatUserName);
          _send('new', {
            'name': name,
            'id': _selfId,
  ...
  Future<void> connect() async {
    ...
    _socket?.onOpen = () async {
      ...
      // _send('new',
      //     {'name': name, 'id': _selfId, 'user_agent': DeviceInfo.userAgent});

      _send('create-new-room', {
        //'roomId': doctorsHPRAdd,
        'roomId': _roomId,
      });
