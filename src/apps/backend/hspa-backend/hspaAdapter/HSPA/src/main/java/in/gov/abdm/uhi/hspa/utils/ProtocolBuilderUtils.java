...
//                objAgent.setImage(obj.profile_photo);
    ...
    private static Catalog extractProviderDetails(Catalog catalog, List<Item> listItems, List<Fulfillment> listFulfillments, Set<Category> listCategory) {
        Descriptor descriptor = new Descriptor();
    ...
    public static Request BuildIntitialization(Request request) {
        ...
        payment.setUri(ConstantsUtils.PAYMENT_URL);
        payment.setType(ConstantsUtils.PAYMENT_STATE_INIT);
        payment.setStatus(ConstantsUtils.PAYMENT_STATUS_INIT);
    ...
    private static List<Breakup> CreateBreakup(Price price) {
        ...
        priceSGST.setValue(sgst.toString());
